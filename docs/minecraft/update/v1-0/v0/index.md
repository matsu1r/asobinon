...
- [slug: /minecraft/update/v1-0/v0/](#slug-minecraftupdatev1-0v0)
- [参考にさせて頂いたページ様](#参考にさせて頂いたページ様)
- [目次](#目次)
- [ワールドの生成(ダンジョンや構造物)に関する追加/変更点](#ワールドの生成ダンジョンや構造物に関する追加変更点)
  - [要塞のエンドポータルから「ジ・エンド(果ての世界)」に行けるようになった](#要塞のエンドポータルからジエンド果ての世界に行けるようになった)
    - [ジ・エンドへの行き方](#ジエンドへの行き方)
    - [これが「ジ・エンド」だ](#これがジエンドだ)
  - [エンドラ討伐後、「エンドゲートウェイポータル」から他の島へワープできる](#エンドラ討伐後エンドゲートウェイポータルから他の島へワープできる)
  - [エンドの島々には「エンドシティ」「エンドシップ」が生成される](#エンドの島々にはエンドシティエンドシップが生成される)
    - [エンドシティ](#エンドシティ)
    - [エンドシップ](#エンドシップ)
  - [雪原/(雪の)タイガバイオームに「イグルー」が生成されるようになった](#雪原雪のタイガバイオームにイグルーが生成されるようになった)
  - [ついにワールドの建築高度制限が「256ブロック」に上がった](#ついにワールドの建築高度制限が256ブロックに上がった)
  - [シード値入力の代わりにMojangが用意した選択肢からシード値を選べるようになった](#シード値入力の代わりにmojangが用意した選択肢からシード値を選べるようになった)
- [モブ(動物、敵キャラ等)に関する追加/変更点](#モブ動物敵キャラ等に関する追加変更点)
  - [ボス「エンダードラゴン」を追加。エンドクリスタルで復活、再戦できるぞ](#ボスエンダードラゴンを追加エンドクリスタルで復活再戦できるぞ)
    - [エンダードラゴンの挙動](#エンダードラゴンの挙動)
    - [倒した後のイベントとドラゴンの卵の回収方法](#倒した後のイベントとドラゴンの卵の回収方法)
    - [ドラゴンの卵の回収方法](#ドラゴンの卵の回収方法)
    - [エンダードラゴンを復活させ、再び戦う方法](#エンダードラゴンを復活させ再び戦う方法)
  - [エンドシティに住む「シュルカー」を追加。浮遊効果の弾を飛ばしてくる/染色可能](#エンドシティに住むシュルカーを追加浮遊効果の弾を飛ばしてくる染色可能)
  - [「エンダーマイト」を追加。エンダーパールを使うとまれにスポーンする](#エンダーマイトを追加エンダーパールを使うとまれにスポーンする)
  - [「シロクマ」を追加。雪原/雪山/樹氷の森バイオームにスポーンする](#シロクマを追加雪原雪山樹氷の森バイオームにスポーンする)
- [ブロックやアイテムの追加、変更点](#ブロックやアイテムの追加変更点)
  - [「エンダーパール」を追加。投げてワープできるぞ](#エンダーパールを追加投げてワープできるぞ)
  - [「エンダーアイ」を追加。要塞の場所を示し、エンドポータルを起動する](#エンダーアイを追加要塞の場所を示しエンドポータルを起動する)
  - [「エンダーチェスト」を追加。どこからでもアイテムを出し入れできる](#エンダーチェストを追加どこからでもアイテムを出し入れできる)
  - [エンダードラゴンの回復役「エンドクリスタル(エンダークリスタル)」を追加、再召喚にも使う](#エンダードラゴンの回復役エンドクリスタルエンダークリスタルを追加再召喚にも使う)
  - [「ドラゴンの卵」を追加](#ドラゴンの卵を追加)
  - [「エンドストーンレンガ」を追加](#エンドストーンレンガを追加)
  - [「コーラスプラント」ブロックを追加。ジ・エンドに生えるコーラスの木の幹](#コーラスプラントブロックを追加ジエンドに生えるコーラスの木の幹)
  - [エンドに生える植物「コーラスフルーツ」を追加。精錬で「焼いたコーラスフルーツ」になる](#エンドに生える植物コーラスフルーツを追加精錬で焼いたコーラスフルーツになる)
  - [「コーラスの花」ブロックを追加。コーラスプラントの種になる](#コーラスの花ブロックを追加コーラスプラントの種になる)
  - [「プルプァブロック」を追加。ハーフ/柱状/階段もあるぞ](#プルプァブロックを追加ハーフ柱状階段もあるぞ)
  - [「シュルカーの殻」を追加。まだボックスはない](#シュルカーの殻を追加まだボックスはない)
  - [「エンドロッド」を追加。空中に浮く便利な照明](#エンドロッドを追加空中に浮く便利な照明)
  - [「エリトラ(翼)」を追加。サバイバルでもすいすいと空を飛べるぞ](#エリトラ翼を追加サバイバルでもすいすいと空を飛べるぞ)
  - [「ドラゴンの頭」を追加](#ドラゴンの頭を追加)
  - [「ドラゴンの息」を追加。ドラゴンのブレスをガラス瓶で採取して入手](#ドラゴンの息を追加ドラゴンのブレスをガラス瓶で採取して入手)
  - [「残留ポーション」を追加。出てきた煙に触れると効果を得られる](#残留ポーションを追加出てきた煙に触れると効果を得られる)
- [ゲームプレイに関する追加/変更点](#ゲームプレイに関する追加変更点)
  - [「locate」コマンドが追加。近くの生成物の座標を確認できる](#locateコマンドが追加近くの生成物の座標を確認できる)
  - [残留ポーションと矢をクラフトして効果付きの矢を作れるようになった](#残留ポーションと矢をクラフトして効果付きの矢を作れるようになった)
  - [ジ・エンドでも地図は使える。](#ジエンドでも地図は使える)
- [システム(ゲーム全体)に関する新要素と変更点](#システムゲーム全体に関する新要素と変更点)
  - [ついにPEにもBGM追加。別途でダウンロードできる](#ついにpeにもbgm追加別途でダウンロードできる)
  - [PEでもWindows10版と同じGUIを使えるオプションを追加](#peでもwindows10版と同じguiを使えるオプションを追加)
  - [チャットの読み上げをON/OFFするオプションを追加](#チャットの読み上げをonoffするオプションを追加)
...
*   ジ・エンドの島の中央にある帰還用ポータルは、最初は起動していない。まずはエンダードラゴンを倒す必要がある
...
![あらかじめ用意されたシード値の選択肢](https://res.cloudinary.com/napoan-com/image/upload/w_650,c_limit,f_auto,q_auto/v1578378576/14ddcb64a631ad37463220a44c072814_tkbsi7.png)
...
*   **[こちらのレシピ](/pe-update-10/2/#crystal)でエンドクリスタルを4つクラフトし、帰還用ポータルの4箇所に図のように設置するとエンダードラゴンと回復用クリスタルが復活する。**
...
### エンダードラゴンの回復役「エンドクリスタル(エンダークリスタル)」を追加、再召喚にも使う
...
![コーラスの花](https://res.cloudinary.com/napoan-com/image/upload/w_650,c_limit,f_auto,q_auto/v1578378673/ea62f8310f0e3166b601222e56e5cf50_fwad8t.png)
...
![エリトラの修理方法](https://res.cloudinary.com/napoan-com/image/upload/w_650,c_limit,f_auto,q_auto/v1578378659/aa0b16100b2688ad9acdc5a4fef03c7b_rmrkip.png)
...
*   Classicを選べば、アイテム一覧画面に加えて、**アイテム検索画面も使えるようになる。**
