...
*   まずは**新アイテムの「[エンダーアイ](#ender-eye)」を投げて、要塞を探そう。**空中でタップすることで投げ、**エンダーアイが地面へゆっくり落ちていくならその下が要塞だ。**
...
*   帰還用ポータルを囲んで**10本の黒曜石の柱**が立っており、上には**エンダードラゴンを回復させる「[エンドクリスタル(エンダークリスタル)](#crystal)」が乗っている**。
...
*   下を壊せば一気に壊せる。**茎のコーラスプラントからは「コーラスフルーツ」がドロップ**する。各アイテムについての詳細は「[コーラスフルーツ](#chorus-fruit)」「[コーラスの花](#chorus-flower)」を参照して欲しい。
...
*   エンドシティは新ブロックの「[エンドストーンレンガ](#end-brick)」や「[プルプァブロック](#purpur)」からできている。
...
*   エンドシティの部屋の中には、チェストと新ブロックの**「[エンダーチェスト](#ender-chest)」がある部屋**もある。
...
*   エンドシティには2種類の塔と4種類の部屋がある。
    *   **小さな塔** – [プルプァハーフブロック](#purpur)がある
    *   **大きな塔** – 大量の[エンドロッド](#end-rod)がある
...
*   船首には新ブロック**「[ドラゴンの頭](#dragon-head)」**が付いている。
...
*   同時に帰還用ポータルが起動し、新ブロック**「[ドラゴンの卵](#dragon-egg)」が設置される。**
...
*   **[こちらのレシピ](#crystal)でエンドクリスタルを4つクラフトし、帰還用ポータルの4箇所に図のように設置するとエンダードラゴンと回復用クリスタルが復活する。**
...
*   倒すと50%の確率で「[シュルカーの殻](#shulker-shell)」をドロップする。
*   エンティティIDはshulker
...
*   エンダードラゴン討伐後、**[こちら](#howto-get-egg)の回収方法で回収**する
...
*   [ジ・エンドの島々](#end-gateway)に生えている、またはプレイヤーが植えて育った**「コーラスプラント」ブロックを破壊**して採取する。
...
*   **[エンドシップ](#endship)の額縁**から入手する。

[ドラゴンの頭](#dragon-head)  
skullのデータ値5

*   **[エンドシップ](#endship)にくっついている**ものを採取する。
...
[**記事はまだ続きます!** 新コマンドや設定などについては3ページ目へ >>](3/)
