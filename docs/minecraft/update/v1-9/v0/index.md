...
slug: /minecraft-je/howto/mcbe-update-19
...
*   [エンティティ(Mob,乗り物など)に関する追加/変更点](#entity)
    *   [(試験的)「略奪者(ピレジャー)」を追加。村を襲撃するぞ](#pillager)
    *   [友好的Mobがランダムにラブモードに入るようになった](#passive)
*   [ワールドの生成(ダンジョンや構造物)に関する追加/変更点](#world)
    *   [(試験的)ジャングルに竹が生えるように](#jungle-bamboo)
*   [ブロックやアイテムの追加、変更点](#block-and-item)
    *   [新しい花「ヤグルマギク」「スズラン(谷間に咲く百合)」を追加](#flowers)
    *   [看板が材料の木材によって色が変わるように](#signs)
    *   [焼石/砂岩/赤砂岩/水晶の「なめらかな」版が追加](#smooth-blocks)
    *   [様々な素材のハーフブロックが追加](#half-blocks)
    *   [様々な素材の階段が追加](#stairs)
    *   [様々な素材の壁が追加](#walls)
    *   [ネザーレンガのフェンスのレシピが変更された](#nether-fence)
    *   [苔石が「苔の生えた丸石」に名前変更](#moss-stone)
    *   [大釜に溶岩を入れられるようになった](#cauldron)
    *   [トリップワイヤーの糸がハサミ以外で壊された時に信号を発するように](#tripwires)
    *   [(試験的)新ブロック「製図台」を追加](#carto)
    *   [(試験的)新ブロック「矢細工台」を追加](#fletching)
    *   [(試験的)新ブロック「石臼」を追加](#grindstone)
    *   [(試験的)新ブロック「ベル」を追加](#bell)
    *   [(試験的)新ブロック「タル」を追加](#barrel)
    *   [(試験的)新ブロック「鍛冶台」を追加](#smithing)
    *   [(試験的)新ブロック「燻製器」を追加](#smoker)
    *   [(試験的)新ブロック「溶鉱炉」を追加](#blast)
    *   [(試験的)新ブロック「ランタン」を追加](#lantern)
*   [ゲームプレイに関する追加・変更点](#gameplay)
    *   [ペットが死んだ時にチャットに通知が流れるように](#pet-die)
*   [コマンドに関する追加・変更点](#command)
    *   [tellrawコマンド実装、JSONテキストを送信できる](#tellraw)
    *   [tagコマンド実装、エンティティにタグを付けられる](#tag)
    *   [死んだら即リスポーンするゲームルールが追加された](#doimmediaterespawn)
    *   [プレイヤーの死亡通知をON/OFFできるようになった](#showdeathmessages)
    *   [particleコマンドで出せるパーティクルの種類が増えた](#particle)
    *   [killコマンドがクリエイティブモードの人にも効くようになった](#kill)
    *   [ファンクションのコマンド実行数を制限するゲームルールが実装](#functioncommandlimit)
*   [システム・総合的なことに関する追加/変更点](#system)
    *   [期限切れのRealmをワールド一覧から削除できるように](#expired)
    *   [ローディング画面のTipsがコミュニティの提案で追加された](#loading)
    *   [新しくなったフィードバックボタンが実装](#feedback)
    *   [遊び方は設定画面で確認できるようになった](#howtoplay)
    *   [ファンクションコマンドが再読込の時点でコンパイルされるように](#function)
...
![](https://cdn-ak.f.st-hatena.com/images/fotolife/s/sasigume/20210208/20210208104852.jpg)
...
*   **死んだ際、「復活」の選択画面を飛ばして即リスポーンする**設定ができるようになった
