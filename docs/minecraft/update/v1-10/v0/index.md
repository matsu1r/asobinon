...
slug: /minecraft-je/howto/mcbe-update-110
...
*   [エンティティ(Mob,乗り物など)に関する追加/変更点](#entity)
    *   [「略奪者」が正式実装。試験的OFFでもスポーンできるように](#pillager)
    *   [ネコのバリエーションに投票で選ばれたジェリーちゃんが追加](#cat)
    *   [飼いネコに生のタラか生鮭を与えることでHPを回復できるように](#cat-hp)
    *   [クリーパーがストレイに倒されてもレコードを落とすように](#creeper)
    *   [\[試験的\] おしゃれで賢い新タイプの村人が試験的に追加](#villager)
    *   [\[試験的\] 村人ゾンビも新バージョンが追加された](#zombie-villager)
    *   [\[試験的\] 「行商人」を追加。ぶらぶらしてるから取引しよう](#trader)
    *   [\[試験的\]「イレジャービースト」が試験的に追加](#beast)
*   [ワールドの生成(ダンジョンや構造物)に関する追加/変更点](#world)
    *   [\[試験的\] 新バージョンの村が試験的実装](#village)
    *   [\[試験的\]「略奪者の拠点」が試験的実装](#outposts)
    *   [要塞が生成される確率が減った](#stronghold)
*   [ブロックやアイテムの追加、変更点](#block-and-item)
    *   [ついに「盾」が実装された](#shield)
    *   [「クロスボウ」がついに正式実装。試験的OFFでも入手可能に](#crossbow)
    *   [新ブロック「書見台」追加。みんなで本を読もう](#lectern)
    *   [新アイテム「旗の模様」追加](#pattern)
    *   [新ブロック「織機」追加。旗を簡単にカスタマイズできる](#loom)
    *   [「ランタン」が正式実装](#lantern)
    *   [「木」ブロックが実装。原木の全部樹皮バージョン](#bark)
    *   [濡れたスポンジがネザーで即乾くように](#sponge)
    *   [葉っぱが消える時に棒を落とすように](#leaves)
    *   [コーラスフルーツが矢で壊せるように](#chorus-fruit)
    *   [ハサミを入れた発射装置でヒツジの毛を刈れるように](#dispensers)
    *   [カーペットが作業台でも染色できるようになった](#carpet)
    *   [ウサギシチューとビートルートスープが不定形レシピに](#stewsoup)
    *   [\[試験的\]「矢細工台」がクラフト可能に](#fletching)
    *   [\[試験的\]「鍛冶台」がクラフト可能に](#smithing)
    *   [\[試験的\] 新ブロック「コンポスター」試験的実装](#composter)
    *   [\[試験的\] 新しくなった「石切台」試験的実装](#stonecutter)
    *   [\[試験的\] 新ブロック「キャンプファイヤー」試験的実装](#campfire)
    *   [\[試験的\] 新植物「甘いベリーの低木」と新食べ物「甘いベリー」試験的実装](#berrie)
    *   [「製図台」がクラフト可能に](#carto)
    *   [「石臼」がクラフト可能に](#grindstone)
    *   [「タル」がクラフト可能に](#barrel)
    *   [「燻製器」がクラフト可能に](#smoker)
    *   [「溶鉱炉」がクラフト可能に](#blast)
*   [コマンド・アドオンに関する追加・変更点](#command)
    *   [Mobの行動のスケジュールをJSONで編集可能に](#mob-json)
    *   [パーティクルをJSONで編集可能に](#particle)
    *   [リソースパックがワールドのテンプレートと分離、独立して更新可能に](#resource)
    *   [エンティティのスクリプティング機能が正式実装](#entity-scripting)
*   [システム・総合的なことに関する追加/変更点](#system)
    *   [新しいバニラテクスチャがデフォルトになった](#texture)
