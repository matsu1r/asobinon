...
*   [エンティティ(Mob,乗り物など)に関する追加/変更点](#entity)
    *   [新タイプの村人が正式実装](#villager)
    *   [新タイプの村人ゾンビが正式実装](#zombie-villager)
    *   [「行商人」が正式実装、付属のラマも進化](#trader)
    *   [邪悪な村人のキャプテンが追加](#captain)
    *   [Illager Beast改めラヴィッジャー追加](#ravager)
    *   [邪悪な村人たちは子供村人を攻撃しなくなった](#illager)
*   [ワールドの生成(ダンジョンや構造物)に関する追加/変更点](#world)
    *   [新バージョンの村が正式実装](#village)
    *   [略奪者の拠点が正式実装](#outposts)
    *   [竹林がついに正式実装](#bamboo-forest)
    *   [シード値セレクターの選択肢が大幅に減った](#seed)
*   [ゲームプレイに関する追加/変更点](#gameplay)
    *   [村人の取引システムが大幅進化](#trading)
    *   [襲撃イベントが実装](#raids)
    *   [邪悪な村人のパトロール実装](#patrols)
    *   [襲撃イベントに関するステータス効果が実装](#effects)
*   [ブロックやアイテムの追加、変更点](#block-and-item)
    *   [矢細工台が正式実装](#fletching)
    *   [鍛冶台が正式実装](#smithing)
    *   [新植物「甘いベリーの低木」と新食べ物「甘いベリー」が正式実装](#berrie)
    *   [書見台でページをめくるとレッドストーン信号を出すように](#lectern)
    *   [足場を支え無しで6ブロック置けるように](#scaffolding)
    *   [かまどにホッパーをつないで精錬した際の経験値が保存されるように](#furnace)
    *   [製図台が正式実装](#carto)
    *   [石臼が正式実装](#grindstone)
    *   [たき火が正式実装、料理機能や煙などが追加](#campfire)
    *   [コンポスターが正式実装](#composter)
*   [コマンド・アドオンに関する追加・変更点](#command)
    *   [mobeventコマンド追加、襲撃やパトロールや行商人を設定可能](#mobevent)
    *   [resupplyコマンド追加、村人の取引アイテムを補充できる](#resupply)
    *   [アドオンの機能がいろいろ改善された](#add-ons)
*   [システム・総合的なことに関する追加/変更点](#system)
    *   [選んだスキンが色々なデバイス間で同期されるように](#skin)
    *   [ワールドを開いた時にチャットの開き方が案内されるように](#chat)
    *   [設定画面に「アクセシビリティ」画面が追加](#accessibility)
    *   [遊び方が村のアップデートに合わせて拡充された](#howtoplay)
    *   [新テクスチャが板ガラスなどより多くのブロック/アイテムに適用された](#texture)
