...
この利用規約（以下，「本規約」といいます。）は、<https://asobinon.org> 及び <https://edit.asobinon.org> で公開されている「アソビノン」（以下，「当サイト」といいます。）がウェブサイト上で提供するサービス（以下，「本サービス」といいます。）の利用条件を定めるものです。登録ユーザーの皆さま（以下，「ユーザー」といいます。）には，本規約に従って，本サービスをご利用いただきます。
...
1. コンテンツの編集にGitHubを使用するため、レポジトリの編集に関わったGitHubアカウントを、1ユーザーとして扱うものとします。
2. 補助サイトでは仮のIDで編集できます。 
...
1. 編集補助サイトにおける支援リンクは、運営個人への支払いであり、なんら公共性はありません。
2. 支払いは[特定商取引法に基づく表示](https://edit.asobinon.org/law)に基づきます。
## 7（利用制限および登録抹消）
